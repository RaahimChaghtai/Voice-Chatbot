...
image = image.pip_install("moshi", "sphn", "fastapi", "huggingface-hub")
...
class Moshi:
    ...
    def setup(self):
        ...
        mimi_weights = hf_hub_download(loaders.DEFAULT_REPO, loaders.MINI_NAME)

        self.mimi = loaders.get_mimi(mimi_weights, self.device)
        self.mimi.set_num_codebooks(8) 
        self.frame_size = int(self.mimi.sample_rate / self.mimi.frame_rate)

        moshi_weights = hf_hub_download(loaders.DEFAULT_REPO, loaders.MOSHI_NAME)

        self.moshi = loaders.get_moshi_lm(moshi_weights, self.device)
        self.lm_gen = LMGen(self.moshi, temperature=0.8, top_k=250)
    ...
    def web(self):
        ...
        @web_app.websocket("/ws")
            async def websocket(ws: WebSocket):
                ...
                try:
                    with torch.no_grad():
                        tasks = [asyncio.create_task(receive()), asyncio.create_task(process()), asyncio.create_task(send())]
